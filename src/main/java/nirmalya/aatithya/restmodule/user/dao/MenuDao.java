...
import nirmalya.aatithya.restmodule.user.model.SelfServiceActivityModel;
...
public class MenuDao {
	...
	public ResponseEntity<JsonResponse<List<Menu>>> getUserMenu(List<String> role, String Org, String OrgDiv) {
		...
		try {
			...
			if (value.length() > 0) {
				...
				String values = "SET @p_val='(" + value + ")',@p_org='" + Org + "',@p_orgDiv='" + OrgDiv + "';";
				logger.info("value===" + values);
				
				List<Object[]> x = em.createNamedStoredProcedureQuery("userRoutines")
						.setParameter("actionType", "getUserMenu1").setParameter("actionValue", values).getResultList();

				for (Object[] m : x) {

					if (m[10] != null) {
						System.out.println("Data === " + m[10].toString());
					}

					Menu menu = new Menu(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9], m[10],m[11]);
					mList.add(menu);
				}
	...
	public ResponseEntity<JsonResponse<List<Menu>>> getFunctionList(UserRolesAndModuleIdModel userModel) {
		...
		try {
			...
			if (value.length() > 0) {
				...
				List<Object[]> x = em.createNamedStoredProcedureQuery("userRoutines")
						.setParameter("actionType", "getFunctionList").setParameter("actionValue", values)
						.getResultList();

				for (Object[] m : x) {

					Menu menu = new Menu(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], null, null, null,null);
					mList.add(menu);
				}
				jsonResponse.setBody(mList);
	...
	public ResponseEntity<JsonResponse<BreadcrumbModel>> getRestBreadcrumbData(String modId, String funId,
		...
		ResponseEntity<JsonResponse<BreadcrumbModel>> response = new ResponseEntity<JsonResponse<BreadcrumbModel>>(
	...
	@SuppressWarnings("unchecked")
	public List<SelfServiceActivityModel> getselfservicelistDao() {
		logger.info("Method : getselfservicelistDao starts");

		List<SelfServiceActivityModel> selfServiceList = new ArrayList<SelfServiceActivityModel>();

		try {
			List<Object[]> x = em.createNamedStoredProcedureQuery("userRoutines")
					.setParameter("actionType", "getSelfServiceList").setParameter("actionValue", "")
					.getResultList();
			for (Object[] m : x) {

				SelfServiceActivityModel selfService = new SelfServiceActivityModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7]);
				selfServiceList.add(selfService);
			}
			
			System.out.println(selfServiceList);
		} catch (Exception e) {
			e.printStackTrace();
		}
