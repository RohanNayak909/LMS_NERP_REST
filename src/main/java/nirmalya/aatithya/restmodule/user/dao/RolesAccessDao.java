...
public class RolesAccessDao {
	...
	public ResponseEntity<JsonResponse<List<RolesAccessModel>>> getRolesList(String org, String orgDiv) {
		...
		try {
			...
			List<Object[]> x = em.createNamedStoredProcedureQuery("userroleRoutines")
					.setParameter("actionType", "getRolesList").setParameter("actionValue", value).getResultList();
	...
	public ResponseEntity<JsonResponse<Object>> saveRoleMaster(RolesAccessModel id) {
		...
		try {
			String values = "SET @P_RoleId='" + id.getRoleId() + "', @P_Status=" + id.getRoleStatus() + 
					", @P_CreatedBy='"+ id.getCreatedBy() +"', @p_org='"+ id.getOrganization() +
					"', @p_orgDiv='"+ id.getOrgDivision() +"', @P_RoleName='" + id.getRoleName() + "', @P_RoleDesc='" + id.getRoleDesc() + "';";
			logger.info("Method : saveRoleMaster starts"+values);
