...
public class RestAccountGroupDao {
	...
	@SuppressWarnings("unchecked")
	public ResponseEntity<JsonResponse<List<RestOrganizationMasterModel>>> getAllAccountGroupList(String orgName,
			String orgDivision) {
			...
			try {
				...
				if (table.getParentName() != null && table.getParentName() != "") {
					...
					em.createNamedStoredProcedureQuery("AccountBankAccountRoutines")
							.setParameter("actionType", "addParent").setParameter("actionValue", values)
							.execute();
				...
				try {
					String[] err = serverDao.errorProcedureCall(e);
					if (err[1].contains("Duplicate Group Name")) {
						resp.setCode("DUPLICATE");
						resp.setMessage("Group name already exists. Insert skipped.");
					} else {
						resp.setCode(err[0]);
						resp.setMessage(err[1]);
					}
	...
	// modify name
	public ResponseEntity<JsonResponse<Object>> modifyHeaderName(String id, String nameGroup, String orgName,
			String orgDiv) {
		...
		if (validity)
			try {
				String value = "SET @p_groupId='" + id + "',@p_groupName='" + nameGroup + "',@p_orgName='" + orgName
						+ "',@p_orgDiv='" + orgDiv + "';";
	...
	public ResponseEntity<JsonResponse<Object>> modifyHeaderNameParent(String id, String nameGroup, String natureGrp,
			String orgName, String orgDiv) {
		...
		if (validity)
			try {
				String value = "SET @p_groupId='" + id + "',@p_groupName='" + nameGroup + "',@p_natureGrp='" + natureGrp
		...
		logger.info("Method :  modifyHeaderNameParent ends" + response);
