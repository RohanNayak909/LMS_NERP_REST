	...
	public static String saveGateReportDataParam(List<ShiftRegisterReportModel> gatereport) {
		...
		if (!gatereport.get(0).getRegisterId().contentEquals("1")) {
			for (ShiftRegisterReportModel m : gatereport) {
				...
				String addman=m.getAddManPower();
				if(addman=="null" || addman==null) {
					addman="";
				}

				listdata = listdata + "(@p_registerId,\"" + m.getCategory() + "\",\"" + m.getInputType() + "\",\"" + m.getManPower()+ "\",\"" + addman+"\",@p_createdBy,@p_organization,@p_orgDivision),";
			}
