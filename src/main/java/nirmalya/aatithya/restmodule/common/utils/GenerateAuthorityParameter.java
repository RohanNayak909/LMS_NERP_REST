...
public class GenerateAuthorityParameter {
	...
	public static String saveProcessMaster(RestProcessMasterModel id) {
		...
		if(id.getOrganization()!=null && id.getOrganization()!="") {
			s = s + "@p_org='" + id.getOrganization() + "',";
		}
		if(id.getOrgDivision()!=null && id.getOrgDivision()!="") {
			s = s + "@p_orgDiv='" + id.getOrgDivision() + "',";
		}
	...
	public static String saveAuthorityMaster(UserSetAuthority id) {
		...
		if(id.getUser()!=null && id.getUser()!="") {
			...
			for(int i = 0; i < arr.length; i++) {
				auth = auth + "(\"" + id.getProcess() + "\",\"" + id.getUserRole() + "\",\"" + arr[i] + "\",@P_StageNo,1,24,\"" + id.getCreatedBy() + "\",\""+id.getDepartment()+"\",\""+id.getOrganization()+"\",\""+id.getOrgDivision()+"\"),";
			}
			...
			String arr[] = id.getUser().split(",");
			for(int i = 0; i < arr.length; i++) {
				auth = auth + "(\"" + id.getProcess() + "\",\"" + id.getUserRole() + "\",\"" + ""+ "\",@P_StageNo,1,24,\"" + id.getCreatedBy() + "\",\""+id.getDepartment()+"\",\""+id.getOrganization()+"\",\""+id.getOrgDivision()+"\"),";
			}
