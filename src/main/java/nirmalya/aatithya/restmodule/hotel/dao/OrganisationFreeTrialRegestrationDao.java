...
@Repository
public class OrganisationFreeTrialRegestrationDao {

	Logger logger = LoggerFactory.getLogger(OrganisationFreeTrialRegestrationDao.class);
	...
	MailService mailService;
	 ...
	 @SuppressWarnings("unchecked")
	    public ResponseEntity<JsonResponse<Object>> freeTrailOrgRegistartion(String orgData) {
	        logger.info("method: freeTrailOrgRegistartion Dao Starts");
	        JsonResponse<Object> resp = new JsonResponse<>();

	        try {
	            JSONObject jsonData = new JSONObject(orgData);

	            // Extract input values
	            String zip = jsonData.optString("zip");
	            ...
	            String orgName = jsonData.optString("orgName");
	            ...
	            String password = generateStrongPassword(firstName, lastName, orgName, mobile);
	            ...
	            String safeMobile = mobile.replace("'", "''");
	            String safeStreet = street.replace("'", "''");
	            ...
	            String safeOrgName = orgName.replace("'", "''");
	            String safePassword = password.replace("'", "''");

	            String value = "SET @email='" + safeEmail + "'," +
	                          "@state='" + safeState + "'," +
	                          "@mobile='" + safeMobile + "'," +
	                          "@street='" + safeStreet + "'," +
	                          "@city='" + safeCity + "'," +
	            ...
	            List<Object[]> resultList = em.createNamedStoredProcedureQuery("hotel_free_trail_routines")
	                .setParameter("actionType", "save-free-trail-registartion")
	                .setParameter("actionValue", value)
	                .getResultList();
	            
	            String subject = "Welcome to Sattkara - Your Organization Registration Details";
	            ...
	            String message = "<html>"
	                ...
	                + "<h1 style='color: #6f42c1;'>Welcome to Sattkara</h1>"
	                + "</div>"
	                ...
	                + "<a href='" + "https://sattkara.nerp.in/" + "' style='display: inline-block; padding: 12px 25px; background-color: #6f42c1; color: white; text-decoration: none; border-radius: 5px; font-weight: bold;'>Login to Your Account</a>"
	                + "</div>"
	                + "<p>If you have any questions or need assistance, please don't hesitate to contact our support team.</p>"
	                + "<p>Best regards,<br><strong>The Sattkara Team</strong></p>"
	                + "</div>"
	                + "<div style='text-align: center; margin-top: 20px; color: #6c757d; font-size: 12px;'>"
	                + "<p>© " + Calendar.getInstance().get(Calendar.YEAR) + " Sattkara. All rights reserved.</p>"
	                + "</div>"
	                + "</div>"
	                + "</body>"
	                + "</html>";
	            ...
	            if (!resultList.isEmpty()) {
	                ...
	                resp.setMessage((String) result[0]);
	                resp.setCode((String) result[1]);
	            } else {
	                resp.setMessage("Unknown response from DB");
	                resp.setCode("Error");
	            }

	        } catch (Exception e) {
	            logger.error("Error in freeTrail Org Registartion: ", e);
	            resp.setMessage("Error saving org data!");
	            resp.setCode("Error");
	        }

	        ResponseEntity<JsonResponse<Object>> response = new ResponseEntity<>(resp, HttpStatus.CREATED);
	        logger.info("method: freeTrailOrgRegistartion Dao Ends");
	        return response;
	    }

	    private String generateStrongPassword(String firstName, String lastName, String orgName, String mobile) {
	        try {
	            String baseString = (firstName.length() >= 3 ? firstName.substring(0, 3) : firstName).toLowerCase() +
	                               (lastName.length() >= 3 ? lastName.substring(0, 3) : lastName).toLowerCase() +
	                               (orgName.length() >= 3 ? orgName.substring(0, 3) : orgName).toLowerCase() +
	            ...
	            for (int i = 0; i < 16; i++) {
	                fallback.append(chars.charAt(random.nextInt(chars.length())));
	            }
