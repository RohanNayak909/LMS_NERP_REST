...
@Repository
public class QaCrqsCheckDao {
	Logger logger = LoggerFactory.getLogger(QaCrqsCheckDao.class);
	...
	public JsonResponse<Object> getAggridDet(String orgName, String orgDivision) {
		...
		try {
			...
			List<Object[]> x = em.createNamedStoredProcedureQuery("qa_crqs_check_routines")
	...
	public ResponseEntity<JsonResponse<List<QaCrqsCheckRestModel>>> addCrqsCheck(List<QaCrqsCheckRestModel> qc) {
		...
		String value = GenerateCrqsCheckParam.getAddCrqsCheck(qc);
		...
		try {
			...
			if (qc.get(0).getCrqscheckId() != null && qc.get(0).getCrqscheckId() != "") {
				...
				em.createNamedStoredProcedureQuery("qa_crqs_check_routines")
