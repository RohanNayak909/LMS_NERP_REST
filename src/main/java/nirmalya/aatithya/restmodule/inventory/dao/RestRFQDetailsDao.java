...
/*	@SuppressWarnings("unchecked")
	...
	public ResponseEntity<JsonResponse<List<DropDownModel>>> getRFQVenListbyRfq(String id) {
		...
		//	logger.info("value at procedure call--------------"+value);
			List<Object[]> x = em.createNamedStoredProcedureQuery("invRFQDtlsRoutines")
	...
	//	logger.info("response vendor time--------------"+response);
		return response;
	}
		...
		if (validation) {
			...
			//	logger.info("value---------------------------------"+value);
				if (RestInventoryRFQVendorDetailModel.get(0).getVenQuotId() != null) {
					//logger.info("I m in EDit section----------------------------");
					...
					//logger.info("I m in add section----------------------------");
					em.createNamedStoredProcedureQuery("invRFQDtlsRoutines")
	...
	public ResponseEntity<JsonResponse<List<RestInventoryRFQVendorDetailModel>>> getRFQVenData(
		...
		//logger.info("value"+request.getParam1());
		String values = GenerateReqQuotParameter.getSearchParam(request);
		Integer total = 0;
		//logger.info("param is Doa -----"+request.getParam1());
		try {
	...
	//	logger.info("Response in Request Quotation VENDOR RFQ dao View==============="+response);
		return response;
	}
...
/*	@SuppressWarnings("unchecked")
	public ResponseEntity<JsonResponse<RestInventoryRFQVendorDetailModel>> getRFQVDById(String id,String action) {
		...
		try {
		    ...
		    logger.info("value for pop up in dao set id is for rfQ------------ : " + value);
		...
		logger.info("response get during edit rfw vendor------"+response);
		return response;
	...
	//	logger.info("response get during edit rfw vendor------"+quotation);
		return quotation;
	}
