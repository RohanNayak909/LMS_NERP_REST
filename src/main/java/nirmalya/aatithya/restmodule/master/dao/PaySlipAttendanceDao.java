...
public class PaySlipAttendanceDao {
	...
	public List<DropDownModel> getEmployedByList(String organization,String orgDivision) {
		...
		try {
			...
			List<Object[]> x = em.createNamedStoredProcedureQuery("payslip_attendance_routines")
					.setParameter("actionType", "getEmployedByList").setParameter("actionValue", value)
					.getResultList();
	...
	public JsonResponse<List<AttendanceDateRestModel>> getEmployeeAttendanceList(String fromDate, String toDate,
			String employedByList, String userId,String organization,String orgDivision,String stafftype,String id) {
...
//				+ DateFormatter.getStringDate(toDate) + "\", @P_EmployedBy=\"" + employedByList + "\", @P_userId=\"" + userId + "\", @P_org=\"" + organization + "\", @P_stafftype=\"" + stafftype + "\", @P_orgDiv=\"" + orgDivision + "\" ,@P_empId=\"" + id +"\";";
//		
		...
		String value = "SET @P_fromDate=\"" + DateFormatter.getStringDate(fromDate) + "\", @P_toDate=\""
		        + DateFormatter.getStringDate(toDate) + "\", @P_userId=\"" + userId + "\", @P_org=\""
		        + organization + "\", @P_orgDiv=\"" + orgDivision + "\", @P_EmployedBy=\"" + employedByList
		        + "\" , @P_StaffType=\"" + stafftype
		        + "\",@p_id='(" + idNew + ")';";

		
		logger.info("value =====>>>>>>>>>>>>>>getAttndListUnapproved>>>>>>>>>" + value);
		try {
			List<Object[]> x = em.createNamedStoredProcedureQuery("payslip_attendance_routines")
			...
			for (Object[] m : x) {
				...
				AttendanceDateRestModel dropDownModel = new AttendanceDateRestModel(m[0], m[1], m[2], m[3], m[4], m[5],
						...
						m[39], m[40].toString(),m[41],m[42],m[43],m[44],m[45].toString(),m[46],m[47].toString(),m[48].toString(),
						m[49].toString(),m[50].toString(),m[51].toString(),m[52].toString());
				stateList.add(dropDownModel);
			}
...
//				}
				AttendanceDateRestModel dropDownModel = new AttendanceDateRestModel(m[0], m[1], m[2], m[3], m[4], m[5],
						...
						m[33].toString(), leave.toString(), m[35].toString(), holiday, m[37].toString(), m[38].toString(),
						m[39], m[40].toString(),m[41],m[42],m[43],m[44],m[45].toString(),m[46],m[47].toString(),m[48].toString(),
						m[49].toString(),m[50].toString(),null,null);
				stateList.add(dropDownModel);
	...
	public ResponseEntity<JsonResponse<List<AttendanceDateRestModel>>> getEmployeeLeaveList(String fromDate,
			String toDate, String employedByList, String userId,String organization,String orgDivision) {
		...
		String value = "SET @P_fromDate=\"" + DateFormatter.getStringDate(fromDate) + "\", @P_toDate=\""
				+ DateFormatter.getStringDate(toDate) + "\", @P_Employee=\"" + employedByList + "\",@P_userId=\"" + userId + "\", @P_org=\"" + organization + "\", @P_orgDiv=\"" + orgDivision +"\";";
