...
public class LocationMasterDao {
	...
	public ResponseEntity<JsonResponse<Object>> saveWarehouseMasterDao(MasterWarehouseModel data) {
		...
		try {
			...
			if (data.getwId() != null && data.getwId() != "") {
				em.createNamedStoredProcedureQuery("masterDataRoutines")
						.setParameter("actionType", "modifyWarehouse").setParameter("actionValue", values).execute();

				resp.setMessage("Data updated successfully");
			} else {
				
				em.createNamedStoredProcedureQuery("masterDataRoutines").setParameter("actionType", "addWarehouse")
	...
	@SuppressWarnings("unchecked")
	public List<DropDownModel> getBedTypeList() {
		logger.info("Method : getBedTypeList starts");

		List<DropDownModel> bedTypeList = new ArrayList<DropDownModel>();

		try {
			List<Object[]> x = em.createNamedStoredProcedureQuery("locationMasterRoutines")
					.setParameter("actionType", "getBedTypeList").setParameter("actionValue", "").getResultList();

			for (Object[] m : x) {
				DropDownModel dropDownModel = new DropDownModel(m[0], m[1]);
				bedTypeList.add(dropDownModel);
			}
		...
		logger.info("Method : getBedTypeList ends");
		return bedTypeList;
	}
